...
export default function Home() {
  return <main className="bw-home">
    ...
    <section className="bw-hero">
      <nav className="bw-nav" aria-label="Primary navigation"><Link href="/" className="brand"><BrandLogo inverse /></Link><div className="bw-nav-links"><a href="#solutions">Solutions</a><a href="#plans">Investment plans</a><a href="#markets">Markets</a></div><div className="bw-nav-actions"><Link href="/sign-in">Log in</Link><Link className="bw-create" href="/create-account">Create account</Link><details className="bw-mobile-menu"><summary aria-label="Open navigation">☰</summary><div><a href="#solutions">Solutions</a><a href="#plans">Investment plans</a><a href="#markets">Markets</a><Link href="/sign-in">Log in</Link></div></details></div></nav>
      <div className="bw-hero-copy"><p>Better Wealth Investment Group</p><h1>Access a more considered <em>investment future.</em> <span>Build your portfolio with confidence.</span></h1><p className="bw-hero-subtitle">Invest in stocks, crypto, ETFs, and commodities from one account. Follow top-performing strategists or chart your own course — Better Wealth Investment Group gives you the tools either way.</p><span className="bw-rule" /><div className="bw-hero-actions"><Link className="bw-light-button" href="/create-account">Explore investments <b>→</b></Link><div><strong>Open an account in minutes</strong><small>Clear next steps, secure client access</small></div></div></div>
    ...
    <section className="bw-portal-section"><svg className="bw-portfolio-growth-line" viewBox="0 0 1440 640" preserveAspectRatio="none" aria-hidden="true"><polyline points="-32,610 148,520 246,568 392,470 486,526 650,376 742,438 872,268 962,324 1104,190 1200,246 1373,95" fill="none" stroke="#075a35" strokeLinecap="square" strokeLinejoin="miter" strokeWidth="24" /><polygon points="1432,15 1411,124 1334,67" fill="#075a35" /></svg><div className="bw-portal-copy"><p>Your client portfolio</p><h2>A private view of<br />what matters.</h2><p>Portfolio value, performance history, asset allocation, and current holdings come together in one secure client view.</p><Link href="/sign-in">Enter client portal <b>↗</b></Link></div><PortfolioPreview /></section>
    <section id="plans" className="bw-plans"><header><p>Investment plans</p><h2>Choose a plan built around your goals.</h2><span>Pick a comfortable portfolio tier during sign-up. Your selected plan follows you into your Better Wealth dashboard.</span></header><div className="bw-plan-grid">{plans.map((plan, index) => <article className={index === 1 ? "bw-plan-featured" : ""} key={plan.name}><div><span>{plan.label}</span></div><h3>{plan.name}</h3><p>{plan.description}</p><section><small>Minimum</small><strong>{plan.minimum}</strong></section><ul>{plan.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><a href="#contact">Choose {plan.name} <b>→</b></a></article>)}</div><small className="bw-plan-note">Plan suitability, fees, minimums, availability, and risk disclosures require approval before publication.</small></section>
    <section id="contact" className="bw-cta"><p>Begin your investment journey</p><h2>Ready to put your<br />capital to work?</h2><span>Open an account in minutes, or speak with Better Wealth about the next appropriate step.</span><div><Link href="/create-account">Create account <b>→</b></Link><a href="mailto:hello@bwiginvest.com">Talk to an advisor</a></div></section>
    <footer className="bw-footer"><Link href="/" className="brand"><BrandLogo inverse /></Link><div><a href="#solutions">Solutions</a><a href="#markets">Markets</a><a href="#plans">Investment plans</a><Link href="/sign-in">Client portal</Link></div><p>© 2026 Better Wealth Investment Group.<br />Trading and investing in stocks, options, and cryptocurrencies involves substantial risk of loss and is not suitable for every investor. You may lose more than your initial investment. Do not invest money you cannot afford to lose.</p></footer>
  </main>;
}
