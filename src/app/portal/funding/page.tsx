...
type RequestItem = { id: string; asset_symbol: string; network: string; status: string; submitted_at: string; kind: "Deposit" | "Withdrawal" };
...
export default function FundingPage() {
  ...
  const [submissionKind, setSubmissionKind] = useState<"deposit" | "withdrawal" | null>(null);
  ...
  async function loadFundingData(preserveMessage = false) {
    ...
    if (!user) { setMessage("Please sign in again to view funding details."); return; }
    const [addressResult, depositResult, withdrawalResult] = await Promise.all([
      supabase.from("deposit_addresses").select("id, asset_symbol, network, public_address").order("assigned_at", { ascending: false }),
      supabase.from("deposit_requests").select("id, asset_symbol, network, status, submitted_at").order("submitted_at", { ascending: false }).limit(3),
      supabase.from("withdrawal_requests").select("id, asset_symbol, network, status, submitted_at").order("submitted_at", { ascending: false }).limit(3),
    ]);
    if (addressResult.error || depositResult.error || withdrawalResult.error) { setMessage("We could not load funding details. Please refresh the page or contact Better Wealth support."); return; }
    ...
    setRequests([...(depositResult.data ?? []).map((request) => ({ ...request, kind: "Deposit" as const })), ...(withdrawalResult.data ?? []).map((request) => ({ ...request, kind: "Withdrawal" as const }))].sort((a, b) => Date.parse(b.submitted_at) - Date.parse(a.submitted_at)));
    if (!preserveMessage) setMessage(assignedAddresses.length ? "" : "No deposit wallet has been assigned to your account yet. Contact Better Wealth support when you are ready to fund.");
  }

  useEffect(() => { void loadFundingData(); }, []);
  ...
  async function submitDeposit(event: FormEvent<HTMLFormElement>) {
    ...
    submitLock.current = true; setSubmissionKind("deposit"); setIsSubmitting(true); setMessage("Deposit reference received. Saving it securely now — please do not submit it again.");
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    ...
    setIsSubmitting(false); setSubmissionKind(null); submitLock.current = false;
    ...
    navigator.vibrate?.(35); event.currentTarget.reset(); setMessage("✓ Deposit reference confirmed. It is now awaiting Better Wealth review, and an email confirmation is on its way."); await loadFundingData(true);
  }
  ...
  async function submitWithdrawal(event: FormEvent<HTMLFormElement>) {
    ...
    submitLock.current = true; setSubmissionKind("withdrawal"); setIsSubmitting(true); setMessage("Withdrawal request received. Saving it securely now — please do not submit it again.");
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
